...
class YOLOHumanTracker:
    ...
    def __init__(self, camera_manager, motor_controller):
        ...
        # Control parameters - REDUCED SPEED FOR SMOOTHER MOVEMENT
        self.max_turn_speed = 40      # Reduced from 50 for smoother turning
        self.max_forward_speed = 45   # Reduced from 50 for gentler movement
    ...
    def _track_human(self, center_x: int, human_height: int, bbox_left: Optional[int] = None, bbox_right: Optional[int] = None):
        ...
        try:
            ...
            distance_deadzone_percentage = 0.03  # 3% of frame height (about 14 pixels)
            distance_deadzone_pixels = distance_deadzone_percentage * self.frame_height
            
            # Apply deadzones
            if abs(x_error) < x_deadzone:
                turn_speed = 0
                logger.debug(f"X_DEADZONE: Centering deadzone applied, x_error={x_error}")
            
            if abs(distance_error_pixels) < distance_deadzone_pixels:
                forward_speed = 0
                logger.debug(f"DIST_DEADZONE: Distance deadzone applied, error={distance_error_pixels:.1f}")
    ...
    def _handle_step_turning(self, desired_turn_speed: float, x_error: int, current_time: float) -> float:
        ...
        if self.is_in_turn_step:
            ...
            if current_time - self.last_turn_step_time >= self.turn_step_duration:
                ...
                # Continue current turn step with gentler speed for longer duration
                turn_speed = self.current_turn_direction * (self.max_turn_speed * 0.4)  # Gentler speed (40%)
            ...
            if current_time - self.last_turn_step_time >= self.turn_pause_duration:
                ...
                if abs(x_error) > 60:  # Still need to turn (outside deadzone)
                    if desired_direction == self.current_turn_direction:
                        # Continue in same direction with gentler speed
